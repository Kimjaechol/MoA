...
export function detectLegalCategory(
  ...
  const categoryPatterns: [RegExp, "민사" | "형사" | "이혼" | "세무" | "행정" | "헌법"][] = [
    ...
    // 이혼/가사
    [/이혼|양육권|친권|위자료|재산분할|별거|혼인|결혼|부부|가정폭력|가사|면접교섭|양육비/, "이혼"],
    // 세무
    [
    ...
    // 헌법
    [/헌법|기본권|위헌|헌법재판|헌법소원|권리침해|국가권력|헌법소원|위헌심판/, "헌법"],
    // 민사 (기본)
    [
...
export async function legalRAG(query: string): Promise<LegalRAGResult> {
  ...
  // 관련 법령 및 판례 검색
  const [laws, precedents] = await Promise.all([searchLaws(query, 5), searchPrecedents(query, 5)]);
