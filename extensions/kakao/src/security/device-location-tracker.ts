 ...
 * === 핵심 원칙 ===
 *
 * GPS 추적은 wipe(데이터 삭제)와 **완전히 독립적**입니다.
 * MoA 데이터를 삭제한 후에도 MoA 앱이 기기에 존재하는 한
 * GPS 좌표를 계속 서버로 전송합니다.
 *
 * 이유: MoA 데이터 외에도 문자, 카톡, 사진, 이메일 등
 * 중요한 개인 데이터가 기기에 그대로 남아있으므로
 * 반드시 기기를 회수해야 합니다.
 *
 * === 추적 종료 조건 (3가지만) ===
 * 1. 사용자가 명시적으로 /추적종료 입력
 * 2. 세션 만료 (기본 72시간)
 * 3. 절취범이 MoA 앱을 삭제 (어쩔 수 없음)
 *
 * wipe 완료는 추적 종료 조건이 아닙니다!
 *
 * === 핵심 흐름 ===
 *
 * 1. 사용자가 /분실신고 입력
 ...
 *    ├─ 30초마다 서버로 좌표 전송
 *    ├─ wipe 실행 (MoA 데이터만 삭제)
 *    └─ wipe 후에도 GPS 전송 계속! (MoA 앱은 살아있으므로)
 *    ↓
